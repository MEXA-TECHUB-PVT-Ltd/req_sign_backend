...
async function createInvoice(
  ...
  return new Promise((resolve, reject) => {
    ...
    writeStream.on("error", (err) => {
      ...
      // return "error"
      reject("error");
      // res.send({error: true, message: 'Error creating PDF'});
    });
    ...
    const imageX = pdfWidth / 2 - imageWidth / 2;
    doc.image("./app/utils/logo.png", imageX, 50, { width: imageWidth });
    const title = "Receipt";
    const customerInformation = "Date\n\nAccount Billed";
    const today = new Date();
    const dd = String(today.getDate()).padStart(2, "0");
    const mm = String(today.getMonth() + 1).padStart(2, "0"); //January is 0!
    const yyyy = today.getFullYear();

    const invoiceDetails = mm + "/" + dd + "/" + yyyy;
    const description = `We received payment for your ${plan_name} subscription. Thanks for your purchase!
  `;
    const customerInformationDetail = `${formattedCurrentToken}\n\n${name} (${email})`;

    doc.fontSize(12).text(description, 50, 160);
    doc.fontSize(18).text(title, 270, 120);
    doc.font("Helvetica-Bold").fontSize(12).text(customerInformation, 50, 190);
    doc
      .fontSize(12)
      .text(customerInformationDetail, 200, 190, { align: "right" });
    doc.moveDown();

    doc.font("Helvetica-Bold").fontSize(13).text("Description", 50, 250);
    doc
      .font("Helvetica-Bold")
      .fontSize(13)
      .text(" Price", 0, 250, { align: "right" });
    doc.strokeColor("lightgrey").moveTo(50, 270).lineTo(550, 270).stroke();

    // List all items
    let i;
    const itemLines = 290;
    const items = [
      { description: "Plan name", price: plan_name },
      { description: "Transaction ID", price: paymentMethodId },
      {
        description: "Amount",
        price: `$${amount} /${duration} (${formattedCurrentToken}-${formattedExpirationToken})`,
      },
      { description: "Tax Amount", price: tax_amount },
      { description: "Members", price: members },
    ];
    for (i = 0; i < items.length; i++) {
      ...
      doc
        .font("Helvetica")
        .fontSize(12)
        .text(item.description, 50, itemLines + i * 25);
      doc
        .font("Helvetica")
        .fontSize(12)
        .text(item.price, 0, itemLines + i * 25, { align: "right" });
    }
    doc
      .strokeColor("lightgrey")
      .moveTo(50, itemLines + i * 25 + 15)
      .lineTo(550, itemLines + i * 25 + 15)
      .stroke();
    ...
    doc
      .font("Helvetica-Bold")
      .fontSize(13)
      .text("Total", 50, itemLines + i * 25 + 30);
    doc
      .font("Helvetica-Bold")
      .fontSize(13)
      .text(`$${total} `, 0, itemLines + i * 25 + 30, { align: "right" });
...
app.get("/create-invoice-func", (req, res) => {
  createInvoice(
    res,
    "Invosaice",
    "22/02/2021",
    "Rimsha Riaz",
    "rimshanimo22@gmail.com",
    "Plan name",
    "20",
    "0",
    "1",
...
app.get("/create-invoice", (req, res) => {
  ...
  //   doc.image(response, 50, 50, { width: 50 });
  // });
  // Add logo
  const pdfWidth = doc.page.width;
  const imageWidth = 200;
  ...
  const invoiceDetails = mm + "/" + dd + "/" + yyyy;
  const description = `We received payment for your GitHub.com subscription. Thanks for your business!
  `;
  // const invoiceDetails = "01/01/2021";
  const customerInformationDetail =
    "Rimsha Riaz\n\nrimshanimo22@gmail.com\n\n031538393\n\nAddress State Postal Code \n\nCountry\n\nCity";

  doc.fontSize(12).text(description, 50, 160);
  doc.fontSize(18).text(title, 270, 120);
  doc.fontSize(12).text(customerInformation, 50, 190);
  // doc.fontSize(10).text(customerInformation, 50, 210);
  doc
    .fontSize(12)
    .text(customerInformationDetail, 200, 190, { align: "right" });
  // doc.fontSize(10).text(`Date: ${invoiceDetails}`, pdfWidth / 2, 120);
  doc.moveDown();

  doc.font("Helvetica-Bold").fontSize(13).text("Description", 50, 350);
  doc
    .font("Helvetica-Bold")
    .fontSize(13)
    .text("Item Price", 0, 350, { align: "right" });
  // Draw a line separating items
  doc.strokeColor("lightgrey").moveTo(50, 370).lineTo(550, 370).stroke();

  // List all items
  let i;
  const itemLines = 390;
  const items = [
    { description: "Plan name", price: "name" },
    { description: "Amount", price: "$15.00" },
    { description: "Tax", price: "$0" },
    { description: "Item 4", price: "$22.00" },
  ];
  for (i = 0; i < items.length; i++) {
    const item = items[i];
    doc
  ...
  doc
    .font("Helvetica-Bold")
    .fontSize(13)
    .text("$50.00", 0, itemLines + i * 25 + 30, { align: "right" });
  doc.pipe(res); // Pipe the document to the response
  doc.end();
});
